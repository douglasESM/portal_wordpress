...
class Topcomm_Widget extends WP_Widget {
  ...
  // start function widget
	function widget( $args, $instance ) {
		...
		$filterEmail = "";
		$countList = "";
		if($instance['minComments'] == "0") {
		} elseif($instance['minComments'] == "") {
		} else {
			$minComments = " HAVING comment_comments >= " . $instance['minComments'];
		}
    ...
    if(count($commenters) > 0) {
      ...
      foreach ($commenters as $k) {
        $url = $wpdb->get_var("SELECT comment_author_url FROM $wpdb->comments
          WHERE $groupBy = '".addslashes($k->$groupBy)."'
        ...
        if(strlen($k->comment_author) > $limitChar) {
          $str = substr($k->comment_author, 0, $limitChar) . "...";
        } else {
  ...
  // start function update widget values
	function update( $new_instance, $old_instance ) {
		...
		$instance['minComments'] = $new_instance['minComments'];
		...
		if( $new_instance['iconAward'] == '' ) {
		  $instance['iconAward'] = 'http://imagizer.imageshack.us/a/img923/4751/wUoZst.png';
		} else {
  ...
  // start function widget form
    ...
    <p>
		<label for="<?php echo $this->get_field_id( 'minComments' ); ?>">List those with a minimum of </label>
		<input id="<?php echo $this->get_field_id( 'minComments' ); ?>" name="<?php echo $this->get_field_name( 'minComments' ); ?>" value="<?php echo $instance['minComments']; ?>" style="width: 30px;" type="text" /> comments
    </p>
    <p>
    ...
    <p>
		...
		<option value=""<?php if ( '' == $instance['defaultGravatar'] ) echo ' selected="selected"'; ?>>Default blue</option>
		...
		<option value="retro"<?php if ( 'retro' == $instance['defaultGravatar'] ) echo ' selected="selected"'; ?>>Retro</option>
		<option value="blank"<?php if ( 'blank' == $instance['defaultGravatar'] ) echo ' selected="selected"'; ?>>Blank</option>
		</select>
    ...
    <p>
		<label for="<?php echo $this->get_field_id( 'displayAward' ); ?>">Award those with at least</label>
		<input id="<?php echo $this->get_field_id( 'displayAward' ); ?>" name="<?php echo $this->get_field_name( 'displayAward' ); ?>" value="<?php echo $instance['displayAward']; ?>" style="width: 30px;" type="text" /> comments
		<br /><small>Award image/icon appears if number greater than zero (0)</small>
    </p>
    <p>
		<label for="<?php echo $this->get_field_id( 'iconAward' ); ?>">Award icon/image file location:</label>
